...
from api.analyzers import typecast, variables, expression
import sys
sys.path.append("../")
...
def analyze(node: Node, lookup_table: dict, executable) -> dict:
    ...
    if children[0].lexeme != children[len(children) - 1].children[0].lexeme:
        raise Exception(
            f"Cannot end without {children[0].lexeme} in the loop.")
    ...
    try:
        ...
        if const.YARN in lookup_table[varname][const.TYPE_KEY]:
            raise Exception(
                f"{typecast.NUMBR(lookup_table[varname][const.VALUE_KEY])} cannot be casted into NUMBR.")
    ...
    while True:
        ...
        update_val = lookup_table[varname][const.VALUE_KEY] + \
            1 if is_increment else lookup_table[varname][const.VALUE_KEY] - 1
        lookup_table[varname] = {
