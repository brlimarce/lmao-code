from api.analyzers import typecast, variables
    ...
    for childindex in range(0, terminal_child):
        ...
        if variables.is_literal(var[1]):
            ...
            if variables.is_exist(var[0], lookup_table):
                vartuple = (
                    lookup_table[var[0]][const.VALUE_KEY], lookup_table[var[0]][const.TYPE_KEY])
        ...
        # Multiplication
        elif var[1] == f"{const.ARITHMETIC_OP} (Multiplication)":
            var1 = exprStack.pop()
            var2 = exprStack.pop()
            checked_values = arith_type_check(var1, var2)
            computedValue = checked_values[0] * checked_values[1]
